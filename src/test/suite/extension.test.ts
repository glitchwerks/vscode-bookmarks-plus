...
suite('Extension - workspace-folder mirror changes', () => {
  test('rapid folder changes finish on the newest workspace instead of racing', async () => {
    let releaseWorkspaceBRead: (() => void) | undefined;
    const workspaceBReadBlocked = new Promise<void>((resolve) => {
      releaseWorkspaceBRead = resolve;
    });
    let workspaceBReadStarted: (() => void) | undefined;
    const didStartWorkspaceBRead = new Promise<void>((resolve) => {
      workspaceBReadStarted = resolve;
    });
    const mirrorA = new FakeMirror();
    const mirrorB = new FakeMirror();
    mirrorB.read = async () => {
      mirrorB.readCount++;
      workspaceBReadStarted!();
      await workspaceBReadBlocked;
      return mirrorB.content;
    };
    const output = new FakeOutput();
    const store = new BookmarkStore(new FakeMemento(), output, {
      mirror: mirrorA,
      writeDelayMs: 5
    });
    let workspaceAResourcesDisposed = false;
    let workspaceBResourcesDisposed = false;

    const coordinator = new WorkspaceMirrorChangeCoordinator(
      store,
      output,
      { dispose: () => { workspaceAResourcesDisposed = true; } },
      undefined,
      {
        createMirror: (location) => {
          assert.strictEqual(location.folder.toString(), vscode.Uri.file('/workspace/b').toString());
          return mirrorB;
        },
        createResources: () => ({ dispose: () => { workspaceBResourcesDisposed = true; } })
      }
    );

    const switchToB = coordinator.rebind([{ uri: vscode.Uri.file('/workspace/b') }]);
    await didStartWorkspaceBRead;
    const disableMirror = coordinator.rebind(undefined);
    releaseWorkspaceBRead!();
    await Promise.all([switchToB, disableMirror]);

    await store.addItem({ type: 'file', uri: 'file:///after-rapid-change.txt' });
    await store.flushMirrorWrites();

    assert.strictEqual(workspaceAResourcesDisposed, true);
    assert.strictEqual(workspaceBResourcesDisposed, true);
    assert.strictEqual(mirrorB.writeCount, 1, 'workspace B is seeded during its transition only');
    assert.ok(output.lines.some((line) => line.includes('no workspace folder is open')));
    coordinator.dispose();
  });

  test('disposing during replacement reconciliation immediately releases provisional resources', async () => {
    let releaseRead: (() => void) | undefined;
    const readBlocked = new Promise<void>((resolve) => {
      releaseRead = resolve;
    });
    let readStarted: (() => void) | undefined;
    const didStartRead = new Promise<void>((resolve) => {
      readStarted = resolve;
    });
    ...
    const coordinator = new WorkspaceMirrorChangeCoordinator(
      ...
      {
        createMirror: () => mirror,
        createResources: () => ({ dispose: () => { resourcesDisposed = true; } })
      }
    );

    const transition = coordinator.rebind([{ uri: vscode.Uri.file('/workspace') }]);
    await didStartRead;
    coordinator.dispose();

    assert.strictEqual(resourcesDisposed, true);
    await store.addItem({ type: 'file', uri: 'file:///after-dispose.txt' });
    releaseRead!();
    await transition;
    await store.flushMirrorWrites();
  ...
  test('disables mirror writes and logs when the workspace becomes multi-root', async () => {
    ...
    await handleWorkspaceFoldersChanged(
      store,
      output,
      [
  ...
  test('no-folder to single-folder attaches a mirror and seeds a missing file', async () => {
    const mirror = new FakeMirror();
    const output = new FakeOutput();
          ...
          items: [
            {
              id: 'workspace-state',
              type: 'file',
              uri: 'file:///workspace/bookmark.txt',
              collectionId: null,
              order: 0
            }
          ],
    ...
    const resources = { dispose: () => undefined };
    ...
    assert.strictEqual(mirror.writeCount, 1, 'workspaceState must seed a missing mirror');
    assert.deepStrictEqual(
      JSON.parse(mirror.content!).items.map((item: { id: string }) => item.id),
      ['workspace-state']
    );
  });

  test('single-folder to no-folder disposes and detaches the old mirror', async () => {
    const mirror = new FakeMirror();
    const output = new FakeOutput();
    const store = new BookmarkStore(new FakeMemento(), output, { mirror, writeDelayMs: 5 });
    let disposed = false;
  ...
  test('changes made while disabled replace a stale mirror when a folder is reopened', async () => {
    const mirror = new FakeMirror();
    const output = new FakeOutput();
    const store = new BookmarkStore(new FakeMemento(), output, {
      mirror,
    ...
    await store.addItem({ type: 'file', uri: 'file:///before-disabled.txt' });
    await store.flushMirrorWrites();
    await handleWorkspaceFoldersChanged(store, output, undefined);
    await store.addItem({ type: 'file', uri: 'file:///while-disabled.txt' });

    await handleWorkspaceFoldersChanged(
      store,
      output,
      [{ uri: vscode.Uri.file('/workspace/reopened') }],
      undefined,
      undefined,
      {
        createMirror: () => mirror,
        createResources: () => ({ dispose: () => undefined })
      }
    );

    assert.deepStrictEqual(
      JSON.parse(mirror.content!).items.map((item: { uri: string }) => item.uri),
      ['file:///before-disabled.txt', 'file:///while-disabled.txt']
    );
  });

  test('an in-flight former-root reload cannot overwrite the replacement workspace', async () => {
    let releaseWorkspaceARead: (() => void) | undefined;
    const workspaceAReadBlocked = new Promise<void>((resolve) => {
      releaseWorkspaceARead = resolve;
    });
    let workspaceAReadStarted: (() => void) | undefined;
    const didStartWorkspaceARead = new Promise<void>((resolve) => {
      workspaceAReadStarted = resolve;
    });
    const mirrorA = new FakeMirror(`${JSON.stringify({
      version: 2,
      items: [{
        id: 'from-a',
        type: 'file',
        uri: 'file:///workspace-a/stale.txt',
        collectionId: null,
        order: 0
      }],
      collections: []
    ...
    mirrorA.read = async () => {
      mirrorA.readCount++;
      workspaceAReadStarted!();
      await workspaceAReadBlocked;
      return mirrorA.content;
    };
    const mirrorB = new FakeMirror(`${JSON.stringify({
      version: 2,
      items: [{
        id: 'from-b',
        type: 'file',
        uri: 'file:///workspace-b/current.txt',
        collectionId: null,
        order: 0
      }],
      collections: []
    ...
    const oldReload = store.reloadFromMirror();
    await didStartWorkspaceARead;
    const switchToB = handleWorkspaceFoldersChanged(
      store,
      new FakeOutput(),
      ...
      undefined,
      {
        createMirror: () => mirrorB,
        createResources: () => ({ dispose: () => undefined })
      }
    );
    await new Promise<void>((resolve) => setTimeout(resolve, 0));
    releaseWorkspaceARead!();
    await Promise.all([oldReload, switchToB]);

    assert.deepStrictEqual(store.getAll().items.map((item) => item.id), ['from-b']);
  });

  test('a local change made during an old-root flush is preserved in the replacement mirror', async () => {
    let releaseWorkspaceAWrite: (() => void) | undefined;
    const workspaceAWriteBlocked = new Promise<void>((resolve) => {
      releaseWorkspaceAWrite = resolve;
    });
    let workspaceAWriteStarted: (() => void) | undefined;
    const didStartWorkspaceAWrite = new Promise<void>((resolve) => {
      workspaceAWriteStarted = resolve;
    });
    const mirrorA = new FakeMirror();
    mirrorA.write = async (content: string) => {
      workspaceAWriteStarted!();
      await workspaceAWriteBlocked;
      mirrorA.content = content;
      mirrorA.writeCount++;
    };
    const mirrorB = new FakeMirror(`${JSON.stringify({
      version: 2,
      items: [{
        id: 'external-b',
        type: 'file',
        uri: 'file:///workspace-b/external.txt',
        collectionId: null,
        order: 0
      }],
      collections: []
    ...
    await store.addItem({ type: 'file', uri: 'file:///before-transition.txt' });
    await didStartWorkspaceAWrite;
    const switchToB = handleWorkspaceFoldersChanged(
      store,
      new FakeOutput(),
      [{ uri: vscode.Uri.file('/workspace/b') }],
    ...
    await store.addItem({ type: 'file', uri: 'file:///during-transition.txt' });
    releaseWorkspaceAWrite!();
    await switchToB;

    assert.deepStrictEqual(
      JSON.parse(mirrorB.content!).items.map((item: { uri: string }) => item.uri),
      ['file:///before-transition.txt', 'file:///during-transition.txt']
    );
  });

  test('single-folder A to B disposes A, adopts B, and sends later writes only to B', async () => {
    const mirrorA = new FakeMirror();
    const mirrorB = new FakeMirror(`${JSON.stringify({
      version: 2,
      items: [
        {
          id: 'from-b',
          type: 'file',
          uri: 'file:///workspace-b/bookmark.txt',
          collectionId: null,
          order: 0
        }
      ],
    ...
    let oldResourcesDisposed = false;
    const oldResources = { dispose: () => { oldResourcesDisposed = true; } };
    const newResources = { dispose: () => undefined };
    let refreshCallCount = 0;
    const refresh = () => { refreshCallCount++; };
    let createdFor: vscode.Uri | undefined;
    let onMirrorEvent: (() => void) | undefined;

    await store.addItem({ type: 'file', uri: 'file:///workspace-a/bookmark.txt' });
    await store.flushMirrorWrites();
    ...
    assert.strictEqual(createdFor?.toString(), vscode.Uri.file('/workspace/b').toString());
    assert.strictEqual(result, newResources, 'the caller must retain workspace B resources');
    assert.deepStrictEqual(store.getAll().items.map((item) => item.id), ['from-b']);

    await store.addItem({ type: 'file', uri: 'file:///workspace-b/after-rebind.txt' });
    await store.flushMirrorWrites();

    assert.strictEqual(mirrorA.writeCount, writesToABeforeRebind);
    assert.deepStrictEqual(
      JSON.parse(mirrorB.content!).items.map((item: { uri: string }) => item.uri),
      ['file:///workspace-b/bookmark.txt', 'file:///workspace-b/after-rebind.txt']
    );
    ...
    mirrorB.content = externalContent;
    const reloaded = new Promise<void>((resolve) => {
      const subscription = store.onBookmarksChanged(() => {
        subscription.dispose();
        resolve();
      });
    });
    assert.ok(onMirrorEvent, 'workspace B watcher callback must be registered');
    onMirrorEvent!();
    await reloaded;

    assert.deepStrictEqual(store.getAll().items.map((item) => item.id), ['external-b']);
  });

  test('a failed replacement reconcile disposes its watcher resources', async () => {
    class RejectingMemento extends FakeMemento {
      update(key: string, value: unknown): Thenable<void> {
        if (key === 'bookmarks.data') {
          return Promise.reject(new Error('simulated state update failure'));
        }
        return super.update(key, value);
      }
    }

    const mirror = new FakeMirror(`${JSON.stringify({
      version: 2,
      items: [{
        id: 'replacement',
        type: 'file',
        uri: 'file:///workspace/replacement.txt',
        collectionId: null,
        order: 0
      }],
      collections: []
    }, null, 2)}\n`);
    const store = new BookmarkStore(new RejectingMemento(), new FakeOutput(), {
      writeDelayMs: 5
    });
    let disposed = false;
    ...
    await assert.rejects(
      () => handleWorkspaceFoldersChanged(
        ...
        {
          createMirror: () => mirror,
          createResources: () => ({ dispose: () => { disposed = true; } })
        }
      ),
      /simulated state update failure/
    );
