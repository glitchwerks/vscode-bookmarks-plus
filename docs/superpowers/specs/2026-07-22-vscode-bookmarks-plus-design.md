...
- **A confirm prompt appears only on a single-root → multi-root transition — not on the empty-window case above, even though that case can also restart the extension host.** The handler asks for confirmation, naming both the mirror-disabling and possible-restart consequences, only when the workspace has exactly one folder before the add (the add would make it the second root; confirm policy decided in issue #56). An empty window (0 folders) proceeds without a prompt — there is no mirror to disable, even though a restart is possible. An already-multi-root workspace (2+ folders) also proceeds without a prompt — the mirror is already disabled and the single-root → multi-root boundary has already been crossed, so adding one more root changes neither consequence.
