...
> **Amended by issue #56.** A workspace-folder change (`onDidChangeWorkspaceFolders`) now also
> triggers `provider.refresh()`, in addition to the mirror handling it already performed —
> covering the case where `bookmarks.addToWorkspace` (see [§5](#5-commands)) changes a global
> folder bookmark's `isInsideWorkspace` result without restarting the extension host. On a
> restart, the reload achieves the same outcome trivially. This is a third invalidation trigger
> alongside the two above, not a replacement for either.
...
**Caveats for `bookmarks.addToWorkspace`.** The handler calls `vscode.workspace.updateWorkspaceFolders(start, null, { uri })`, `start` being the current folder count, to append the bookmarked folder as a new workspace root. The consequences below are pre-existing VS Code and mirror behavior that this command triggers, not new behavior invented for #56:

- **The extension host may restart.** `updateWorkspaceFolders`'s own contract warns: "in some cases calling this method may result in the currently executing extensions (including the one that called this method) to be terminated and restarted. For example when the first workspace folder is added, removed or changed… Another case is when transitioning from an empty or single-folder workspace into a multi-folder workspace" — `node_modules/@types/vscode/index.d.ts:13916-13920` (the pinned `@types/vscode ^1.85.0` dev dependency, `package.json:102`). Both named trigger cases apply here: an empty window has no first folder yet, so adding one *is* "the first workspace folder is added" — plainly the most likely restart case, and it proceeds with **no confirm prompt** (see below); a single-root workspace gaining a second root is the "transitioning… into a multi-folder workspace" case. The method's return value only reports whether the operation *started*, not whether a restart follows, so the handler cannot detect or await one. The workspace-folder-change refresh added by issue #56 (see [§2](#2-architecture)) covers the non-restart case; a restart satisfies the same visible outcome trivially by reloading the window.
- **The `.vscode/bookmarks.json` mirror is disabled on a single-root → multi-root transition.** This is the pre-existing mirror rule described in the README's "The `.vscode/bookmarks.json` mirror" section — the mirror requires exactly one workspace folder — reacting to the same `onDidChangeWorkspaceFolders` event this command fires, not new logic added for #56. The handler also calls `flushMirrorWrites()` before `updateWorkspaceFolders`, so a pending debounced mirror write is not lost to a mid-write extension-host restart.
- **A confirm prompt appears only on a single-root → multi-root transition — not on the empty-window case above, even though that case can also restart the extension host.** The handler asks for confirmation, naming both the mirror-disabling and possible-restart consequences, only when the workspace has exactly one folder before the add (the add would make it the second root; plan `docs/superpowers/plans/2026-08-15-global-bookmarks-and-add-to-workspace.md` §8 Q9). An empty window (0 folders) proceeds without a prompt — there is no mirror to disable, even though a restart is possible. An already-multi-root workspace (2+ folders) also proceeds without a prompt — the mirror is already disabled and the single-root → multi-root boundary has already been crossed, so adding one more root changes neither consequence.
- **The command is visible in the command palette but not usable from it.** Unlike `addFile`/`addFolder`/`addFileGlobal`/`addFolderGlobal`/`newGlobalCollection` above, `bookmarks.addToWorkspace` has no `when: false` entry under `contributes.menus.commandPalette` in `package.json`, so VS Code lists it in the palette. Invoked there, the command runs with no tree node argument, and the handler's own guard (`node.kind !== 'item' || …`) assumes a node is always supplied — this is a gap in the shipped contribution, not a deliberate design choice, and is being tracked as a follow-up rather than fixed in this docs-only change.
