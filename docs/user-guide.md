...
- **Show Full Path** switches between a flat filename list and an expandable folder hierarchy
  ...
  descendants. With repository grouping enabled, the hierarchy does not repeat the repository
  name. Targets outside every attached root fall back to filename-only. The setting persists
